...
class NativeWebSite extends NativeOleControlSite {
	WebSite wrapperWebSite;
...
public NativeWebSite(NativeComposite parent, int style, String progId) {
...
int GetHostInfo(long pInfo) {
	...
	IE browser = (IE)((NativeBrowser)((Browser)getParent().getParent()).getWrappedWidget()).webBrowser;
	...
	*/
	...
	if (msg.message == OS.WM_KEYDOWN) {
		switch ((int)msg.wParam) {
			case OS.VK_F5:
				OleAutomation auto = new OleAutomation(this.wrap());
...
int ProcessUrlAction(long pwszUrl, int dwAction, long pPolicy, int cbPolicy, long pContext, int cbContext, int dwFlags, int dwReserved) {
	...
	*/
	if (dwAction == IE.URLACTION_FEATURE_ZONE_ELEVATION) {
		IE ie = (IE)((NativeBrowser)((Browser)getParent().getParent()).getWrappedWidget()).webBrowser;
...
boolean canExecuteApplets () {
	...
	*/
	...
	if (canExecuteApplets == null) {
		WebBrowser webBrowser = ((NativeBrowser)((Browser)getParent().getParent()).getWrappedWidget()).webBrowser;
...
@Override
protected WebSite wrap() {
	return wrapperWebSite;
}
