 ...
 */
final public class NativeOleFrame extends NativeComposite
{
	OleFrame wrapperOleFrame;
	...
	private NativeOleClientSite currentdoc;
 ...
 * </ul>
 */
public NativeOleFrame(NativeComposite parent, int style) {
	super(parent, style);
...
private static void initCheckFocus (final Display display) {
	...
	timer[0] = () -> {
		if (lastFocus[0] instanceof NativeOleClientSite && !lastFocus[0].isDisposed()) {
		...
		if (lastFocus[0] == null || lastFocus[0].isDisposed() || !lastFocus[0].isFocusControl()) {
			...
			if (currentFocus instanceof NativeOleFrame) {
				NativeOleFrame frame = (NativeOleFrame) currentFocus;
			...
			if (lastFocus[0] != currentFocus) {
				...
				if (currentFocus instanceof NativeOleClientSite && !currentFocus.isDisposed()) {
...
private static void initMsgHook(Display display) {
	...
	final Callback callback = new Callback(NativeOleFrame.class, "getMsgProc", 3); //$NON-NLS-1$
...
static long getMsgProc(long code, long wParam, long lParam) {
	...
	if (OS.WM_KEYFIRST <= message && message <= OS.WM_KEYLAST) {
		if (display != null) {
			...
			if (widget instanceof NativeOleClientSite) {
				NativeOleClientSite site = (NativeOleClientSite)widget;
				if (site.handle == hwnd) {
					...
					if (!rc || (lpgui.flags & mask) == 0) {
						NativeOleFrame frame = site.frame;
...
NativeOleClientSite getCurrentDocument() {
	return currentdoc;
}
void setCurrentDocument(NativeOleClientSite doc) {
	currentdoc = doc;
...
@Override
protected OleFrame wrap() {
	return wrapperOleFrame;
}
