...
class WidgetWin32Tests extends Win32AutoscaleTestBase {
	...
	public void testWidgetZoomShouldChangeOnZoomLevelChange() {
		...
		NativeButton button = Widget.checkNative(new Button(shell.getWrapper(), SWT.PUSH));
	...
	public void testButtonPointsAfterZooming() throws NoSuchMethodException, IllegalAccessException {
		...
		shell = Widget.checkNative(new Shell(display));
	...
	public void testCoolItemAfterZooming() {
		...
		NativeCoolBar coolBar = Widget.checkNative(new CoolBar(shell.getWrapper(), SWT.NONE));
		NativeCoolItem item1 = Widget.checkNative(new CoolItem(coolBar.getWrapper(), SWT.NONE));
		NativeLabel label1 = Widget.checkNative(new Label(coolBar.getWrapper(), SWT.NONE));
	...
	public void testExpandItemAfterZooming() {
		...
		NativeExpandBar coolBar = Widget.checkNative(new ExpandBar(shell.getWrapper(), SWT.NONE));
		NativeExpandItem item1 = Widget.checkNative(new ExpandItem(coolBar.getWrapper(), SWT.NONE));
		NativeLabel label1 = Widget.checkNative(new Label(coolBar.getWrapper(), SWT.NONE));
		label1.setText("Label 1");
		item1.setControl(label1);
		item1.setHeight(10);
	...
	public void testTabFolderSizeAfterZooming() {
		...
		NativeTabFolder tabFolder = Widget.checkNative(new TabFolder(shell.getWrapper(), SWT.NONE));
		tabFolder.setBounds(20, 20, 360, 240);

		NativeTabItem tabItem = Widget.checkNative(new TabItem(tabFolder.getWrapper(), SWT.NONE));
		tabItem.setText("Tab 1");
		NativeLabel label = Widget.checkNative(new Label(tabFolder.getWrapper(), SWT.NONE));
	...
	public void testTableAfterZooming() {
		...
		NativeTable table = Widget.checkNative(new Table(shell.getWrapper(), SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL));
		table.setBounds(20, 20, 360, 240);
		table.setHeaderVisible(true);
		table.setLinesVisible(true);

		NativeTableColumn column = Widget.checkNative(new TableColumn(table.getWrapper(), SWT.NONE));
		column.setText("Column 1");
		column.setWidth(200);

		Font font = new Font(display, "Arial", 12, SWT.BOLD);

		NativeTableItem item1 = Widget.checkNative(new TableItem(table.getWrapper(), SWT.NONE));
	...
	public void testTreeAfterZooming() {
		...
		NativeTree tree = Widget.checkNative(new Tree(shell.getWrapper(), SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL));
		tree.setBounds(20, 20, 360, 240);
		tree.setHeaderVisible(true);
		tree.setLinesVisible(true);

		NativeTreeColumn column = Widget.checkNative(new TreeColumn(tree.getWrapper(), SWT.NONE));
		column.setText("Column 1");
		column.setWidth(200);

		Font font = new Font(display, "Arial", 12, SWT.BOLD);
		NativeTreeItem item1 = Widget.checkNative(new TreeItem(tree.getWrapper(), SWT.NONE));
		item1.setText("Item 1");
		item1.setFont(font);
