	...
	*/
	...
	if (checkSelection) {
		if (wasSelected || forceSelect) {
			...
			if (index != -1) event.item = _getItem (index).wrap();
	...
	switch ((int)wParam) {
		...
		case SWT.CR:
			...
			*/
			...
			if (index != -1) {
				Event event = new Event ();
				event.item = _getItem (index).wrap();
...
//			if (drawCount != 0 || !OS.IsWindowVisible (handle)) break;
		...
		case OS.LVN_ITEMACTIVATE: {
			...
			if (pnmlv.iItem != -1) {
				Event event = new Event ();
				event.item = _getItem (pnmlv.iItem).wrap();
				sendSelectionEvent (SWT.DefaultSelection, event, false);
			}
			break;
		}
...
private static void handleDPIChange(Widget widget, int newZoom, float scalingFactor) {
	...
	for (NativeTableColumn tableColumn : table.getColumns()) {
		DPIZoomChangeRegistry.applyChange(tableColumn.wrap(), newZoom, scalingFactor);
	}
