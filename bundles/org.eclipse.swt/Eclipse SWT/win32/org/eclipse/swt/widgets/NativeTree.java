...
NativeTreeItem _getItem (long hItem, int id) {
	...
	return id != -1 ? items [id] : new TreeItem (new NativeTreeItem(this, SWT.NONE, -1, -1, hItem)).getWrappedWidget();
}
...
void setItemCount (int count, long hParent) {
	...
	if (itemDeleteFrom != 0) {
		...
		if ((style & SWT.VIRTUAL) != 0) {
			...
			for (int i = 0; i < numInserted; i++) {
				new TreeItem (new NativeTreeItem(this, SWT.NONE, hParent, itemInsertAfter, 0)).getWrappedWidget();
			}
		}
	}
	if (redraw) {
	...
	switch ((int)wParam) {
		...
		case SWT.CR: {
			...
			*/
			Event event = new Event ();
			long hItem = OS.SendMessage (handle, OS.TVM_GETNEXTITEM, OS.TVGN_CARET, 0);
			if (hItem != 0) event.item = _getItem (hItem).wrap();
		...
		case OS.VK_END: {
			...
			if (OS.GetKeyState (OS.VK_SHIFT) < 0) {
				...
				if (hItem != 0) {
					...
					OS.SendMessage (handle, OS.TVM_GETITEM, 0, tvItem);
					Event event = new Event ();
					event.item = _getItem (hNewItem, (int)tvItem.lParam).wrap();
					sendSelectionEvent (SWT.Selection, event, false);
					return new LRESULT (code);
				}
			}
...
LRESULT WM_LBUTTONDBLCLK (long wParam, long lParam) {
	...
	if (lpht.hItem != 0) {
		if ((style & SWT.CHECK) != 0) {
			if ((lpht.flags & OS.TVHT_ONITEMSTATEICON) != 0) {
				...
				OS.NotifyWinEvent (OS.EVENT_OBJECT_FOCUS, handle, OS.OBJID_CLIENT, (int)id);
				Event event = new Event ();
				event.item = _getItem (tvItem.hItem, (int)tvItem.lParam).wrap();
				event.detail = SWT.CHECK;
				sendSelectionEvent (SWT.Selection, event, false);
				return LRESULT.ZERO;
			}
	...
	if (lpht.hItem != 0) {
		...
		if ((lpht.flags & flags) != 0) {
			Event event = new Event ();
			event.item = _getItem (lpht.hItem).wrap();
			sendSelectionEvent (SWT.DefaultSelection, event, false);
		}
	}
	return result;
...
private static void handleDPIChange(Widget widget, int newZoom, float scalingFactor) {
	...
	for (NativeTreeColumn treeColumn : tree.getColumns()) {
		DPIZoomChangeRegistry.applyChange(treeColumn.wrap(), newZoom, scalingFactor);
	}
