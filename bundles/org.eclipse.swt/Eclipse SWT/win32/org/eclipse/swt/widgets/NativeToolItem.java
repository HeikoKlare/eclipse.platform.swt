...
boolean isTabGroup () {
	NativeToolItem [] tabList = parent._getTabItemList ();
	if (tabList != null) {
...
void selectRadio () {
	int index = 0;
	NativeToolItem [] items = parent.getNativeItems ();
...
void updateImages (boolean enabled) {
	...
	if (info.iImage == OS.I_IMAGENONE) {
		Rectangle bounds = DPIUtil.scaleBounds(image.getBounds(), Widget.checkNative(getParent()).getZoom(), 100);
...
@Override
public abstract ToolItem getWrapper();
