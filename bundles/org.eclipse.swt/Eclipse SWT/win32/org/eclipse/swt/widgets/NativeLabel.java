...
public class NativeLabel extends NativeControl<Label> {
	...
	static {
		...
		DPIZoomChangeRegistry.registerHandler(NativeLabel::handleDPIChange, Label.class);
	}
...
boolean mnemonicHit (char key) {
	...
	while (control.parent != null) {
		NativeControl [] children = control.parent._getChildren ();
		int index = 0;
...
private static void handleDPIChange(Widget widget, int newZoom, float scalingFactor) {
	if (!(Widget.checkNative(widget) instanceof NativeLabel label)) {
		return;
	}
