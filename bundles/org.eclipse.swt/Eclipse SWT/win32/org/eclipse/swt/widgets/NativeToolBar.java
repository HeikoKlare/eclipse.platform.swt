...
public abstract class NativeToolBar extends NativeComposite implements IToolBar {
	...
	static {
		...
		DPIZoomChangeRegistry.registerHandler(NativeToolBar::handleDPIChange, ToolBar.class);
	}
...
NativeWidget computeTabGroup () {
	NativeToolItem [] items = _getItems ();
	if (tabItemList == null) {
		int i = 0;
...
NativeWidget [] computeTabList () {
	...
	NativeToolItem [] list = tabList != null ? _getTabItemList () : items;
	for (NativeToolItem child : list) {
	...
	if (result.length == 0) result = new NativeWidget [] {this};
	return result;
}
...
void createItem (NativeToolItem item, int index) {
	int count = (int)OS.SendMessage (handle, OS.TB_BUTTONCOUNT, 0, 0);
	if (!(0 <= index && index <= count)) error (SWT.ERROR_INVALID_RANGE);
	int id = 0;
	while (id < items.length && items [id] != null) id++;
	if (id == items.length) {
		NativeToolItem [] newItems = new NativeToolItem [items.length + 4];
...
void destroyItem (NativeToolItem item) {
	TBBUTTONINFO info = new TBBUTTONINFO ();
	info.cbSize = TBBUTTONINFO.sizeof;
...
public ToolItem getItem (int index) {
	...
	return items [lpButton.idCommand].getWrapper();
}
...
NativeToolItem getItemInPixels (Point point) {
	for (NativeToolItem item : getNativeItems ()) {
...
NativeToolItem [] _getItems () {
	int count = (int)OS.SendMessage (handle, OS.TB_BUTTONCOUNT, 0, 0);
	TBBUTTON lpButton = new TBBUTTON ();
	NativeToolItem [] result = new NativeToolItem [count];
	for (int i=0; i<count; i++) {
		OS.SendMessage (handle, OS.TB_GETBUTTON, i, lpButton);
...
NativeToolItem [] _getTabItemList () {
	if (tabItemList == null) return tabItemList;
	int count = 0;
	for (NativeToolItem item : tabItemList) {
		if (!item.isDisposed ()) count++;
	}
	if (count == tabItemList.length) return tabItemList;
	NativeToolItem [] newList = new NativeToolItem [count];
	int index = 0;
	for (NativeToolItem item : tabItemList) {
		if (!item.isDisposed ()) {
			newList [index++] = item;
...
/*public*/ void setTabItemList (NativeToolItem [] tabList) {
	checkWidget ();
	if (tabList != null) {
		for (NativeToolItem item : tabList) {
			if (item == null) error (SWT.ERROR_INVALID_ARGUMENT);
			if (item.isDisposed ()) error (SWT.ERROR_INVALID_ARGUMENT);
			if (item.parent != this) error (SWT.ERROR_INVALID_PARENT);
		}
		NativeToolItem [] newList = new NativeToolItem [tabList.length];
...
LRESULT wmCommandChild (long wParam, long lParam) {
	NativeToolItem child = items [OS.LOWORD (wParam)];
	if (child == null) return null;
	return child.wmCommandChild (wParam, lParam);
}

int getForegroundPixel (NativeToolItem item) {
...
int getBackgroundPixel (NativeToolItem item) {
...
private static void handleDPIChange(Widget widget, int newZoom, float scalingFactor) {
	if (!(Widget.checkNative(widget) instanceof NativeToolBar toolBar)) {
		return;
	}
	NativeToolItem[] toolItems = toolBar._getItems();
	...
	record ToolItemData(NativeToolItem toolItem, TBBUTTON button) {
	}
	...
	for (int i = itemCount - 1; i >= 0; i--) {
		...
		NativeToolItem item = toolItems[i];
	...
	while (!buttondata.isEmpty()) {
		...
		NativeToolItem item = itemData.toolItem;
		if (item != null) {
...
@Override
public abstract ToolBar getWrapper();
