...
public class NativeText extends NativeScrollable {
	...
	static {
		...
		DPIZoomChangeRegistry.registerHandler(NativeText::handleDPIChange, Text.class);
	}
...
void applySegments () {
	...
	OS.SendMessage (handle, OS.EM_SETLIMITTEXT, limit + Math.min (nSegments, Text.LIMIT - limit), 0);
	length += nSegments;
...
public int getTextLimit () {
	checkWidget ();
	int limit = (int)OS.SendMessage (handle, OS.EM_GETLIMITTEXT, 0, 0) & 0x7FFFFFFF;
	if (segments != null && limit < Text.LIMIT) limit = Math.max (1, limit - segments.length);
	return limit;
}
	...
	switch (key) {
		case 0x08:	/* Bs */
			if (start [0] == end [0]) {
				...
				if (start [0] == lineStart) {
					start [0] = start [0] - Text.DELIMITER.length ();
				} else {
					start [0] = start [0] - 1;
				}
		...
		case 0x7F:	/* Del */
			if (start [0] == end [0]) {
				...
				if (end [0] == lineStart - Text.DELIMITER.length ()) {
					end [0] = end [0] + Text.DELIMITER.length ();
				} else {
					end [0] = end [0] + 1;
				}
...
public void setTextLimit (int limit) {
	...
	if (segments != null && limit > 0) {
		OS.SendMessage (handle, OS.EM_SETLIMITTEXT, limit + Math.min (segments.length, Text.LIMIT - limit), 0);
	} else {
...
private static void handleDPIChange(Widget widget, int newZoom, float scalingFactor) {
	if (!(Widget.checkNative(widget) instanceof NativeText text)) {
		return;
	}
	text.setMargins();
}
