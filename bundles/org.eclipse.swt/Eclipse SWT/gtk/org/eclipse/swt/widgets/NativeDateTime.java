...
void createDropDownButton () {
	...
	popupListener = event -> {
		if (event.widget == popupShell.wrap()) {
			popupShellEvent (event);
			return;
		}
		if (event.widget == popupCalendar.wrap()) {
			popupCalendarEvent (event);
			return;
		}
		if (event.widget == NativeDateTime.this.wrap()) {
			onDispose (event);
			return;
		}
		if (event.widget == getShell ().wrap()) {
	...
	popupFilter = event -> {
		...
		if (shell == NativeDateTime.this.getShell ().wrap()) {
			handleFocus (SWT.FocusOut);
		}
	};
}
...
void createPopupShell (int year, int month, int day) {
	popupShell = new Shell (getShell ().wrap(), SWT.NO_TRIM | SWT.ON_TOP).getWrappedWidget();
	int popupStyle = SWT.CALENDAR;
	if (showWeekNumbers()) {
		popupStyle |= SWT.CALENDAR_WEEKNUMBERS;
	}
	popupCalendar = new DateTime (popupShell.wrap(), popupStyle).getWrappedWidget();
	...
	mouseEventListener = event -> {
		if (event.widget instanceof Control) {
			...
			if (c != down.wrap() && c.getShell () != popupShell.wrap())
...
void dropDownCalendar (boolean drop) {
	...
	if (getShell ().wrap() != popupShell.getParent ()) {
		recreateCalendar ();
	}
...
void popupCalendarEvent (Event event) {
	switch (event.type) {
		case SWT.Dispose:
			if (popupShell != null && !popupShell.isDisposed () && !isDisposed () && getShell ().wrap() != popupShell.getParent ()) {
				int year = popupCalendar.getYear ();
				int month = popupCalendar.getMonth ();
				int day = popupCalendar.getDay ();
				popupShell = null;
