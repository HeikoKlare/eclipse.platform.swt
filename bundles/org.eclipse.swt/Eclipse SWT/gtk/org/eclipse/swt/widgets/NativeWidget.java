...
void checkOrientation (NativeWidget parent) {
	style &= ~SWT.MIRRORED;
...
void hookEvents () {
	if (handle != 0) {
		OS.g_signal_connect (handle, OS.dpi_changed, display.notifyProc, NativeWidget.DPI_CHANGED);
	}
}
...
char [] sendIMKeyEvent (int type, long event, char [] chars) {
	...
	long ptr = 0;
	if (event == 0) {
		long controller = NativeControl.getControl(this.handle).keyController;
...
void sendSelectionEvent (int eventType, Event event, boolean send) {
	...
	if (ptr != 0) {
		...
		currentEventType = NativeControl.fixGdkEventTypeValues(currentEventType);
...
int gdk_pointer_grab (long gdkResource, int grab_ownership, boolean owner_events, int event_mask, long confine_to, long cursor, int time_) {
	...
	if (gdkSeatGrabPrepareFunc == null) {
		gdkSeatGrabPrepareFunc = new Callback(NativeWidget.class, "GdkSeatGrabPrepareFunc", 3); //$NON-NLS-1$
	}
...
public abstract Widget getWrapper();
