...
public class NativeToolBar extends NativeComposite {
	...
	static Callback menuItemSelectedFunc = new Callback(NativeToolBar.class, "MenuItemSelectedProc", 2);
...
NativeToolItem[] _getItems () {
	if (GTK.GTK4) {
		ArrayList<NativeToolItem> childrenList = new ArrayList<>();
		for (long child = GTK4.gtk_widget_get_first_child(handle); child != 0; child = GTK4.gtk_widget_get_next_sibling(child)) {
			NativeWidget childWidget = display.getWidget(child);
			if (childWidget != null) {
				childrenList.add((NativeToolItem)childWidget);
			}
		}

		return childrenList.toArray(new NativeToolItem[childrenList.size()]);
	} else {
		long list = GTK3.gtk_container_get_children (handle);
		if (list == 0) return new NativeToolItem [0];
		int count = OS.g_list_length (list);
		NativeToolItem [] items = new NativeToolItem [count];
		long originalList = list;
		int index = 0;
		for (int i=0; i<count; i++) {
			long data = OS.g_list_data (list);
			NativeWidget widget = display.getWidget (data);
			if (widget != null) items [index++] = (NativeToolItem) widget;
			list = OS.g_list_next (list);
		}
		OS.g_list_free (originalList);
		if (index != items.length) {
			NativeToolItem [] newItems = new NativeToolItem [index];
...
long menuItemSelected (long widget, NativeToolItem item) {
