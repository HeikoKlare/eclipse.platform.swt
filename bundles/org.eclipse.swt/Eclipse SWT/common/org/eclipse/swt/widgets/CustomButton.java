 ...
 */
public class CustomButton extends CustomNativeBasedControl implements IButton {
	...
	private Button[] getRadioGroup() {
		...
		for (int k = 0; k < children.length; k++) {
			if (children[k].wrap() instanceof Button b
...
// TODO this must be improved.
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		if (text != null && !text.isEmpty()) {
			GC gc = new GC(this);
			gc.setFont(getFont());
			var textExtent = gc.textExtent(text, DRAW_FLAGS);
