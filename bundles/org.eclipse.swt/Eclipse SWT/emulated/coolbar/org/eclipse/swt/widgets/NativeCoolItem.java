...
void updateChevron() {
	if (control != null) {
		...
		if ((style & SWT.DROP_DOWN) != 0 && width < preferredWidth) {
			if (chevron == null) {
				chevron = new ToolBar (parent.getWrapper(), SWT.FLAT | SWT.NO_FOCUS).getWrappedWidget();
				ToolItem toolItem = new ToolItem (chevron.getWrapper(), SWT.PUSH);
